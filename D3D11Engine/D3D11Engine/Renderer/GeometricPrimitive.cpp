...
_Use_decl_annotations_
void GeometricPrimitive::Impl::Draw(ID3D11DeviceContext* mdeviceContext)
{
	unsigned int stride;
	unsigned int offset;
...
GeometricPrimitive::GeometricPrimitive() : pImpl(new Impl())
{
...
void GeometricPrimitive::Render(D3D11RendererMaterial* pMaterial)
{
	ID3D11DeviceContext* pDeviceContext = g_objDeviecManager.GetImmediateContext();
	pDeviceContext->IASetPrimitiveTopology((D3D_PRIMITIVE_TOPOLOGY)D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	pDeviceContext->IASetInputLayout(pMaterial->getLayout<VertexPositionNormalTexture>());
	pImpl->Draw(pDeviceContext);
}
...
std::unique_ptr<GeometricPrimitive> GeometricPrimitive::CreateCylinder( float height /*= 1*/, float diameter /*= 1*/, size_t tessellation /*= 32*/, bool rhcoords /*= true*/)
{
	VertexCollection vertices;
	IndexCollection indices;
...
std::unique_ptr<GeometricPrimitive> GeometricPrimitive::CreateOctahedron( float size /*= 1*/, bool rhcoords /*= true*/)
{
	VertexCollection vertices;
	IndexCollection indices;
