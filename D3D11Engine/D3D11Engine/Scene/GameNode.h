...
typedef std::shared_ptr<class GameNode> SPNode;
typedef std::weak_ptr<class GameNode> WPNode;

typedef std::vector<SPNode> VEC_SPNODE;

typedef std::shared_ptr<class Component> SPComponent;
typedef std::weak_ptr<class Component> WKComponent;

typedef std::vector<SPComponent> VEC_SP_COMPONENT;
typedef std::vector<Component*> VEC_WP_COMPONENT;
...
//	HpNodePtr CreateChild(const String& name , unsigned id =0);
	...
	GameNode* CreateChild(unsigned id, CreateMode mode);
	/// Create a child scene node (with specified ID if provided).
	GameNode* CreateChild(const String& name = "", CreateMode mode = REPLICATED, unsigned id = 0);

	void AddChild(SPNode node, unsigned index = M_MAX_UNSIGNED);
	...
	GameNode* GetParent() const { return parent_; }
	...
	const VEC_SPNODE& GetChildren() const { return children_; }
	int GetNumChildren() const{ return children_.size(); }

	/// Return child scene node by index.
	GameNode* GetChild(unsigned index) const;
	/// Return child scene node by name.
	GameNode* GetChild(const String& name, bool recursive = false) const;
	/// Return child scene node by name.
	GameNode* GetChild(const char* name, bool recursive = false) const;
	/// Return child scene node by name hash.
	GameNode* GetChild(StringHash nameHash, bool recursive = false) const;
	...
	void RemoveChild(VEC_SPNODE::iterator iter);
	...
	void RemoveComponent(VEC_SP_COMPONENT::iterator i);
...
private:
	...
	VEC_SPNODE children_;
	/// Components.
	VEC_SP_COMPONENT components_;
	VEC_WP_COMPONENT  listeners_;

};

template <class T>
T* GameNode::GetComponent(bool recursive /*= false*/) const
{
	return static_cast<T*>(GetComponent(T::GetTypeStatic(), recursive));
}

template <class T>
T* GameNode::CreateComponent(CreateMode mode /*= REPLICATED*/, unsigned id /*= 0*/)
{
