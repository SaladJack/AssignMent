...
Scene::~Scene()
{
	...
	for (HashMap_NODE_ITERATOR i = replicatedNodes_.begin(); i != replicatedNodes_.end(); ++i)
		i->second->ResetScene();
	for (HashMap_NODE_ITERATOR i = localNodes_.begin(); i != localNodes_.end(); ++i)
		i->second->ResetScene();
...
void Scene::NodeAdded(GameNode* node)
{
	...
	{
		HashMap_NODE_ITERATOR iter = replicatedNodes_.find(id);
	...
	{
		HashMap_NODE_ITERATOR  iter = localNodes_.find(id);
	...
	const VEC_SPNODE & children = node->GetChildren();
	VEC_SPNODE::const_iterator iter = children.begin();
	for (; iter != children.end(); ++iter)
		NodeAdded((*iter).get());

}

void Scene::NodeRemoved(GameNode* node)
{
	if (!node || node->GetScene() != this)
		return;
	unsigned id = node->GetID();
...
void Scene::MarkReplicationDirty(GameNode* node)
{
	unsigned id = node->GetID();
...
GameNode* Scene::GetNode(unsigned id) const
{
	if (id < FIRST_LOCAL_ID)
	{
		HashMap_NODE_ITERATOR i = replicatedNodes_.find(id);
		return i != replicatedNodes_.end() ? i->second : 0;
	}
	else
	{
		HashMap_NODE_ITERATOR i = localNodes_.find(id);
...
void Scene::ComponentAdded(Component* component)
{
	...
	if (id < FIRST_LOCAL_ID)
	{
		HashMap_Component_Iter i = replicatedComponents_.find(id);
	...
	{
		HashMap_Component_Iter i = localComponents_.find(id);
